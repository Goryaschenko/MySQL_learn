...
/* 10. Создаем таблицу постов пользователя
 * 
 * Сценарий Вася добавляет пост с картинкой на свою страницу ВК.
 * 
 * Вася выкладывает пост с картинкой
 * Пост может быть приатной или нет
 * 
 */
...
/*11. Создаем таблицу  Лайки на посты пользователей, лайки на медиафайлы
 * 
 *В таблице храним:
 *user_id пользователя лайкнувшкго пост
 *owner_id пользователя держателя поста
 *media_id поста или файла
 * 
 */
CREATE TABLE likes (
 ...
 * В таблице хранятся:
 * user_id - пользователь владелец черного списка
 * scoundrel_id - негодяй, подлец, мерзавец, шельма
 * 
 * 
 * 
 */
...
user_id BIGINT UNSIGNED NOT NULL,
scoundrel_id BIGINT UNSIGNED NOT NULL,
PRIMARY KEY (scoundrel_id, user_id),
